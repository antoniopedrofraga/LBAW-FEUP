...
function getPageFromIndex(userid) {
	...
	$.getJSON("../ajax/get-myauctions.php", { id: userid }, function(data) {
		...
		for (auction; auction < limit; auction++) {
			...
			output += '<h4><span id="clock-' + data[auction].idleilao + '"></span> (data de inicio: ' + data[auction].datainicio + ')</h4>';
...
$( document ).ready(function() {
	...
	$('body').on('click', '.remove-auction', function (){
		var auctionId = $(this).attr('id');
		$.ajax({
			url: "../ajax/delete-auction.php",
			method: "POST",
			data: { auctionid : auctionId, auctionerid : id },
		}).done(function( msg ) {
			page = 0;
			getPageFromIndex(id);
		}).fail(function( jqXHR, textStatus ) {
			alert( "Não foi possível apagar o leilão" );
		});
