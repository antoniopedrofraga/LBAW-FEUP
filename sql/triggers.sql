...
CREATE OR REPLACE FUNCTION atualizaMaiorLicitacao ( _idMembroBanido INTEGER )
 	RETURNS void AS
$$
	DECLARE
    	_leilao INTEGER;
	...
	BEGIN
		FOR _leilao IN (SELECT idLeilao FROM Leilao)
  		LOOP
    		UPDATE Leilao SET licitacaoAtual = (SELECT licitacaoAtual FROM Licitacao WHERE idLeilao = _leilao ORDER BY valor DESC LIMIT 1) 
    		WHERE idLeilao = _leilao RETURNING nome INTO _nomeLeilao;
    		/*IF (SELECT idCliente FROM Licitacao WHERE idLeilao = _leilao ORDER BY valor DESC LIMIT 1) IS NOT NULL THEN 
    			INSERT INTO Notificacao (idUtilizador, texto) VALUES ((SELECT idCliente FROM Licitacao WHERE idLeilao = _leilao ORDER BY valor DESC LIMIT 1)
    				, 'A sua licitacao no leilao ' || _nomeLeilao || ' voltou a ser a mais alta.');
    		END IF;*/
  		END LOOP;
	END;
$$ LANGUAGE plpgsql;
...
$$
	BEGIN
		...
		EXECUTE atualizaMaiorLicitacao ( NEW.idMembroBanido );
		RETURN NEW;
	END;
$$ LANGUAGE plpgsql;
